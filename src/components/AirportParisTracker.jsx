// src/components/AirportParisTracker.jsx
...
export default function AirportParisTracker({ onNeedsUpdate, utilityItems = {} }) {
    ...
    const calculateNeeds = useCallback(() => {
        ...
        slots.forEach(slot => {
            if (!slot.completed && slot.quantity > 0 && slot.item.trim() !== '') {
                ...
                // Uses unique key 'airportParis' for App.jsx aggregation
                needs[itemName] = (needs[itemName] || 0) + slot.quantity;
            }
        });
        onNeedsUpdate({ airportParis: needs });
    }, [slots, onNeedsUpdate]);
    ...
    return (
        ...
        <div className="utility-card airport-paris-tracker">
            <h3>Airport (Paris)</h3>
